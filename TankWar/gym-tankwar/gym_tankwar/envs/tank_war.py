...
class TankWar(gym.Env):
    ...
    def __init__(self, render_mode: str | None = None, starting_hp: int = 3) -> None:
        ...
        self.angles = (0, 90, 180, 270)  # All possible angles of the tanks and bullets
        self.player_speed = 4  # The moving speed of the player based on a framerate of 30
        self.max_enemies = 4  # The maximum number of enemies
        self.max_enemy_bullets = 20  # The maximum number of enemies' bullets
        self.enemy_killed_reward = 1  # The reward when the player kills an enemy
        self.player_killed_reward = -5  # The reward when the player is killed by the enemies

        # Observation is a 51-element normalized list: the player's location, a max. of 4 enemies' locations, a max. of 20 enemy's bullets' locations, player's gun's remaining reloading time
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        if len(self.enemies) > 0:
            enemies_loc = np.concatenate([enemy.get_location() for enemy in self.enemies])
            enemies_loc = np.pad(enemies_loc, (0, self.max_enemies * 2 - len(enemies_loc)), "constant", constant_values=(-1,))
        else:
            enemies_loc = np.full((self.max_enemies * 2,), -1)
        if len(self.enemy_bullets) > 0:
            enemy_bullets_loc = np.concatenate([bullet.get_location() for bullet in self.enemy_bullets])
            enemy_bullets_loc = np.pad(enemy_bullets_loc, (0, self.max_enemy_bullets * 2 - len(enemy_bullets_loc)), "constant", constant_values=(-1,))
        else:
            enemy_bullets_loc = np.full((self.max_enemy_bullets * 2,), -1)
        player_gun_reload_time = 0 if self.player.last_shoot == 0 else max(0, 1 - (self.steps - self.player.last_shoot) / (self.metadata["render_fps"] * 1))
        player_gun_reload_time = np.array([player_gun_reload_time])
        obs = np.concatenate([player_loc, enemies_loc, enemy_bullets_loc, player_gun_reload_time], dtype=np.float32)
        return obs
    ...
    def _create_player(self) -> None:
        """An internal function that creates one player at a random location in the middle of the window."""
        ...
        self.player = Player(player_start_x, player_start_y, player_start_angle, self.window_width, self.window_height, self._fps_to_speed(self.player_speed))
    ...
    def _create_enemy(self) -> tuple[int, float]:
        """An internal function that creates sufficient enemies at random locations on the borders."""
    ...
    def step(self, action):
        ...
        for enemy in self.enemies:
            ...
            # Rotates an enemy with an interval of not less than 3 seconds and a probability of 1% (based on a framerate of 30)
            if self.steps - enemy.last_rotate >= self.metadata["render_fps"] * 3 and self.np_random.random() < self._fps_to_prob(0.01):
                enemy_angle = self.np_random.choice([angle for angle in self.angles if angle != enemy.angle])
            ...
            enemy_touches_border, enemy_correction_angles = enemy.update(enemy_dx, enemy_dy, enemy_angle)
        ...
        """Step 9: Terminate the episode if the player has collided with any of the enemies or any of the enemies' bullets"""
        
        if pygame.sprite.spritecollide(self.player, self.enemies, dokill=True) or pygame.sprite.spritecollide(self.player, self.enemy_bullets, dokill=True):
    ...
    def _fps_to_prob(self, original_prob: float) -> float:
        """An internal function that converts the original probability, which was based on a framerate of 30, to a probability that fits any framerate."""

        return 30 * original_prob / self.metadata["render_fps"] 

    def _player_shoot(self, angle: int) -> None:
        """An internal function that controls how the player shoots. The player can shoot at the beginning or for every one second."""

        if self.player.last_shoot == 0 or self.steps - self.player.last_shoot >= self.metadata["render_fps"] * 1:
            self.player.last_shoot = self.steps
            player_bullet = self.player.bullet(self.player.surf.get_size(), self.player.rect.center, angle, self.player_speed + self._fps_to_speed(3))
    ...
    def _enemy_shoot(self, enemy: Enemy, angle: int, interval: int) -> None:
        """An internal function that controls how an enemy shoots. It shoots with an interval of not less than 2 seconds and a probability of 5% (based on a framerate on 30)."""

        if len(self.enemies) < self.max_enemy_bullets and self.steps - enemy.last_shoot >= self.metadata["render_fps"] * interval and self.np_random.random() < 0.05:
            enemy.last_shoot = self.steps
            enemy_bullet = enemy.bullet(enemy.surf.get_size(), enemy.rect.center, angle, enemy.speed + self._fps_to_speed(2))
    ...
    def _render_frame(self) -> np.ndarray | None:
        ...
        if self.player.last_shoot != 0:
            reload_bar_len = max(0, 80 * (self.metadata["render_fps"] * 1 - (self.steps - self.player.last_shoot)) // self.metadata["render_fps"])
